    ...
    try {
      if (interaction.options.getSubcommand() === "anime") {
        ...
        if (res.episodes === null) {
          res.episodes = "No episode count found.";
        }
        let time =
          `${res.season.toTitleCase()} ${res.startDate.year}` ||
          "To be announced.";
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({
            text: "Last Updated",
            iconURL:
              "https://www.gitbook.com/cdn-cgi/image/width=50,height=50,fit=contain,dpr=1,format=auto/https%3A%2F%2F553834213-files.gitbook.io%2F~%2Ffiles%2Fv0%2Fb%2Fgitbook-legacy-files%2Fo%2Fspaces%252F-LHizcWWtVphqU90YAXO%252Favatar.png%3Fgeneration%3D1531944291782256%26alt%3Dmedia",
          });
        ...
        if (res.status === "NOT_YET_RELEASED") {
          ...
          embed.addFields([
            {
              name: "Status",
              value: `${res.status.toTitleCase()}`,
              inline: true,
            },
            {
              name: "Average Score",
              value: `${res.meanScore} out of 100`,
              inline: true,
            },
          ]);
        }
      ...
      if (interaction.options.getSubcommand() === "manga") {
        ...
        if (res.chapters === null) {
          res.chapters = "No chapter count found.";
        }
        if (res.volumes === null) {
          res.volumes = "No volume count found.";
        }
